...
int main(int argc, char *argv[]) {
    ...
    char *src_client_vendor = NULL;
    char *src_client_product = NULL;
    ...
    {
        printf("-v and -p are mandatory options.\n");
        return 1;
    }

    // Ensure sender and receiver are passed
    if(argc - optind != 2) {
        printf("both sender and receiver must be specified.\n");
        return 1;
    }

    // Retrieve sender and receiver
    char *src_client_name = argv[optind];
    char *dest_client_name = argv[optind + 1];
    ...
    for (;;) {
        ...
        if (src_client_id > -1) {
            fprintf(stderr, "Found Client '%s' with ID %d.\n", src_client_name, src_client_id); 
